...
function activateShipit() {
  ...
  ShipitDebug::debug('Shipit user ');
  ShipitDebug::debug(get_option('shipit_user')['shipit_user']);
...
if (in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins')))) {
  ...
  function getMeasures($height = 0, $width = 0, $length = 0, $weight = 0) {
    ...
    ShipitDebug::debug('el cart');
    ShipitDebug::debug(print_r($cart,true));
    foreach ($cart as $cartItem) {
      ShipitDebug::debug('el caritem');
      ShipitDebug::debug(print_r($cartItem,true));
      $product = getProduct($cartItem);
      ShipitDebug::debug('el producto');
      ShipitDebug::debug(print_r($product,true));
