  ...
  class Core {
    ...
    function massivePackages($packages = array()) {
      ...
      if (is_wp_error($response)) {
        ...
        $package = json_decode($response['body']);
        $this->bugsnag->bugsnagLog($package, "package mass create service", $reference);
      }
      return $package;
    }
    ...
    function orders($order = array()) {
      ...
      $this->bugsnag->bugsnagLog($order, "core orders service", $reference);
      return $order;
    }
    ...
    function shipments($shipment = array()) {
      ...
      if (is_wp_error($response)) {
        echo 'Error al conectar con API.';
        $this->bugsnag->bugsnagWpLog($response, "shipments service", $reference);
      } else {
        $shipment = json_decode($response['body']);
        $this->bugsnag->bugsnagLog($shipment, "shipments service", $reference);
      }    
      return $shipment;
    }
