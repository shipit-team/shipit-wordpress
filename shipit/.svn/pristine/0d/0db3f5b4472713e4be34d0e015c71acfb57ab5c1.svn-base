        ...
        function enroll_shipit( $order_id) {
            ...
            if( ! get_post_meta( $order_id, '_thankyou_action_done', true ) ) {
                ...
                if ($order->status != 'cancelled' && $order->status != 'failed' && $order->status != 'on-hold' && $order->status != 'refunded' && $order->status != 'pending' && $order->status != 'pending payment') {
                    ...
                    $order = new WC_Order($order_id );
                    $order->add_order_note('no pudo ser enviado a Shipit por que el pedido todavia no esta confirmado o esta fallido'); 
                }
                                    ...
                                    'packing_set' => array(
                                        ...
                                        'weight_set' => array(
                                            ...
                                            'calculate_shiping' => array(
                                                ...
                                                'price-setup' => array(
                                                    'title' => __( 'configuracion de Valor', 'woocommerce' ),
                                                    'type' => 'price',
                                                    'description' => __( 'Configure su valor a las comunas %.', 'woocommerce' ),
                                                    'css'      => 'max-width:200px;',
                                                ),
                                                'free_communes'    => array(
                                                    'title'             => __( 'Comunas especificas', 'woocommerce' ),
                                        ...
                                        public function calculate_shipping( $package = array() ) {
                                            ...
                                            if(is_object($ship) && $ship->state == 'error' || !$ship){
                                                ...
                                                $info = ($shows->settings['communes'] != '')  ? in_array(strval( $commune_id ) , $shows->settings['communes'], TRUE) : 0;
                                                $free = ($shows->settings['free_communes'] != '') ? in_array(strval( $commune_id ) , $shows->settings['free_communes'], TRUE) : 0;
                                                $free = ($shows->settings['free_communes_for_price'] != '' && $upper == true) ? in_array(strval( $commune_id ) , $shows->settings['free_communes_for_price'], TRUE) : 0;
                            ...
                            function shipit_cURL_wrapper_request($version, $url, $method,  $order_id = null, $commune_id = null ) {
                                ...
                                if ('POST' == $method) {
                                            ...
                                            $body = [
                                                ...
                                                $body = [
                                                    ...
                                                    if($config_shipit->configuration->automatic_delivery === false){
                                                        ...
                                                        $request_params['order']['seller']['name'] = '';
                                                        $request_params['order']['seller']['id'] = '';
                                                        $request_params['order']['seller']['reference_site'] = '';
                                                        ...
                                                        $request_params['order']['sizes']['width'] = $body_request->packing_measures->width;
                                                        $request_params['order']['sizes']['height'] = $body_request->packing_measures->height;
                                                        $request_params['order']['sizes']['length'] = $body_request->packing_measures->length;
                                                        $request_params['order']['sizes']['weight'] = $body_request->packing_measures->weight;
                                                        $request_params['order']['sizes']['volumetric_weight'] = $body_request->packing_measures->weight;
                                                        ...
                                                        $request_params['order']['courier']['client'] = null;
                                                        $request_params['order']['prices'] = array();
                                                        $request_params['order']['prices']['total'] = 0;
                                                        $request_params['order']['prices']['price'] = 0;
                                                        $request_params['order']['prices']['cost'] = 0;
                                                        $request_params['order']['prices']['insurance'] = 0;
                                                        $request_params['order']['prices']['tax'] = 0;
                                                        ...
                                                        $request_params['order']['origin']['street'] = ($address['street'] != '') ? $address['street'] : $order->get_shipping_address_1();
                                                        $request_params['order']['origin']['number'] = $address['number'];
                                                        $request_params['order']['origin']['complement'] = $order->get_shipping_address_2();
                                                        $request_params['order']['origin']['commune_id'] = (int) filter_var($order->get_shipping_state(), FILTER_SANITIZE_NUMBER_INT);
                                                        $request_params['order']['origin']['full_name'] = $order->get_shipping_first_name() . ' ' . $order->get_shipping_last_name();
                                                        $request_params['order']['origin']['email'] = $order->get_billing_email();
                                                        $request_params['order']['origin']['phone'] = $order->get_billing_phone();
                                                        $request_params['order']['origin']['store'] = false;
                                                        $request_params['order']['origin']['origin_id'] = null;
                                                        $request_params['order']['origin']['name'] = 'predeterminado';
                                                        ...
                                                        $request_params['order']['destiny']['complement'] = $order->get_shipping_address_2();
                                    ...
                                    function split_street($streetStr) {
                                        ...
                                        $aMatch         = array();
                                        $pattern        = '#^([\w/áéíóúüñÁÉÍÓÚÜÑ[:punct:] ]+) ([0-9]{1,5})((,.)[\w/áéíóúüñÁÉÍÓÚÜÑ[:punct:]\-/]*)$#';
